...
The `/usage` page shows one card per meter with its allowance, reset date, remaining units, and reservations; selecting a card loads its 30-day series and per-member breakdown, and a meter with no activity in the window shows an empty state instead of a blank chart. The `/dashboard` API activity chart reads the same endpoint for `api.requests` over 14 days.
