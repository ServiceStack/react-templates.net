...
3. Configure account audience, public visibility, description, order, contact-sales behavior, and trial days.
...
`MyApp/plans.json` seeds these records only for an empty database. The default catalog includes shared Free, Individual-only Personal, and Business-only Pro, Business, and Enterprise plans. Plan audience is part of each published version's contract.
...
A self-service paid Price must be active, positive, published, available to the active account kind, and mapped to a Stripe `price_...` identifier. Free plans require no Stripe Price. Contact-sales plans intentionally bypass self-service Checkout. The pricing page offers Personal and Business plan tabs above the cards, with monthly/annual billing alongside; signed-in Checkout remains bound to the active workspace kind.

![Multi-Currency and Interval Plan Prices](/img/next-saas/plan-prices.png)

**Create missing in Stripe** saves the current plan as a draft when needed, including unsaved edits, before provisioning. Stripe catalog provisioning uses plan metadata and idempotency keys. It saves returned Price mappings directly into the draft but never publishes a plan automatically. Live provisioning requires explicit opt-in.
