...
SQLite is the frictionless local default and allows the whole template to recreate from an empty file. It is also supported for a first single-host deployment check when the durable `App_Data` volume is mounted. The default production policy, `Deployment.RequireNetworkDatabase`, requires a networked database server.

Deployment selects the provider with one repository variable rather than a code change. [Choose a database](/docs/next-saas/operations/choose-a-database) covers the switch, its Kamal destination layout, and how to add another RDBMS.
