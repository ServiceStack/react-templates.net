...
For local screenshots and product tours, `./scripts/seed-example-data.sh` writes the same
Development-only example dataset through OrmLite on every supported provider. It does not alter
the production migration seed, is safe to rerun, and refuses non-Development environments.
