...
Deployment.RequireExplicitMigrations=true
```
...
The schema must contain the Identity and SaaS tables. The plan query must return Free, Pro, Business, and Enterprise.
...
Sign in with the temporary bootstrap administrator. Confirm `/admin`, `/admin/plans`, and the public pricing page load. Create a disposable customer user and confirm a new personal organization is created.
