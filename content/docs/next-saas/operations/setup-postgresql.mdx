...
```bash
cp config/appsettings.deploy.postgres.example.json \
...
```bash
./scripts/reset-kamal-deployment.sh --service next-saas --provider postgres
```
...
```bash
./scripts/reset-kamal-deployment.sh \
  --service next-saas \
  --provider postgres \
  --yes
```
