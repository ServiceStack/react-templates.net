...
| Pre-deploy hook | none | `config/db/postgres/pre-deploy.sh` | `config/db/mysql/pre-deploy.sh` | `config/db/sqlserver/pre-deploy.sh` |
...
- **It enforces a password policy** — at least eight characters drawn from three of uppercase,
  ...
  `openssl rand -base64 48 | tr -dc 'A-Za-z0-9' | head -c 40`. It also needs roughly 2GB of memory,
  which rules out the smallest hosts.
