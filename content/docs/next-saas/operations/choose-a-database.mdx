...
description: "SQLite is the default. Switching to PostgreSQL or another RDBMS is a one-variable configuration change that applies to local development and production alike, not a pipeline fork."
---

[Operations](/docs/next-saas/operations) · [Choose your database](/docs/next-saas/getting-started/choose-your-database) · [Database and storage](/docs/next-saas/operations/database-and-storage) · [Deployment](/docs/next-saas/operations/deployment)

[Choose your database](/docs/next-saas/getting-started/choose-your-database) is the step-by-step
guide for a developer setting the template up. This page is the operator reference behind it.
...
- **It enforces a password policy** — at least eight characters drawn from three of uppercase,
  ...
  refuses to start, so `configure-deployment.sh` rejects it first. Generate one with:
...
- **It runs no initializer scripts**, unlike the PostgreSQL and MySQL images. Its pre-deploy hook
  waits for the server to accept connections and then applies an idempotent `init.sql` through
  `sqlcmd`, and creates the data directory owned by uid 10001 because the image cannot write to a
...
1. add its OrmLite and EF Core packages, a `Database:Provider` branch, and a
   `ConfigureDb.TryNormalizeProvider` name in `MyApp/Configure.Db.cs`;
...
- [Choose your database](/docs/next-saas/getting-started/choose-your-database)
