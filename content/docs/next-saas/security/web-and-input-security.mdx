...
ServiceStack's built-in operator UIs are Vue applications that compile templates at runtime with `new Function()`, which CSP treats as eval. They therefore need `'unsafe-eval'`, which the customer-facing policy deliberately withholds. `Security.ToolingPaths` lists the paths that receive the relaxed policy, defaulting to `/admin-ui`, `/ui`, and `/metadata`; every other route keeps the strict policy unchanged. The relaxed policy is derived from `Security.ContentSecurityPolicy` by adding `'unsafe-eval'` to its `script-src` so the two cannot drift apart, and `Security.ToolingContentSecurityPolicy` overrides that derivation. Path matching is by segment, so `/admin-uix` is not treated as `/admin-ui`.

Do not add `'unsafe-eval'` to the main policy to fix an operator UI. That would extend eval to the customer-facing application, which is the surface actually exposed to untrusted content.
