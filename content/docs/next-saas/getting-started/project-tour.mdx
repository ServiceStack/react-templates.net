...
`MyApp/Migrations/Migration1000.cs` creates the SaaS schema and seeds `plans.json`. Because this template is still under heavy development, a clean database is the expected way to apply structural changes here. A derived live product should use new numbered migrations.
...
- `app/signup` offers Personal and Business account types; `app/pricing` lets visitors switch between their plan catalogs above the plan cards, alongside the monthly/annual billing selector.
- `app/admin` contains the role-gated, static Operations Center routes.
- `components/app-shell.tsx` switches between organization navigation and capability-aware Operations Center navigation. Below the `lg` breakpoint it shows a bottom tab bar whose **More** sheet holds every page, the organization switcher, and sign-out. It also exports the shared `PageHeading`, `Panel`, `StatusPill`, and threshold-aware `Meter` primitives.
- `components/brand.tsx` holds the product mark and theme toggle shared by the public site and the application shell.
