...
Add the feature and a `ReportsGenerated` seed property to the appropriate entries in `MyApp/plans.json`, then extend `Migration1000`’s `PlanSeed`, `SeedPlan`, and quota insertion to persist `reports.generated`.
