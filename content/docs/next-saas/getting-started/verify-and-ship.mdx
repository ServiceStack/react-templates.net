...
<CopyBlock>{`./scripts/preflight.sh --config-only`}</CopyBlock>

The checked-in SQLite profile deliberately relaxes only the networked-database, SMTP, and Stripe requirements and reports warnings. Do not broadly disable checks outside an intentional deployment profile.

## 6. Apply migrations explicitly

An empty database bootstraps itself on first start. Later migrations are a release step, applied before starting multiple application instances:
...
<CopyBlock>{`dotnet publish MyApp/MyApp.csproj -c Release`}</CopyBlock>
