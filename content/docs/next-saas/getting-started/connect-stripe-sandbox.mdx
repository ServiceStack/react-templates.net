...
Alternatively, put them in the gitignored `.env` beside the solution, which the application applies automatically in Development:
...
A variable already set in your shell still wins over the file. See [Configuration](/docs/next-saas/operations/configuration#the-private-env-in-development).
...
3. Confirm monthly and annual amounts and currencies, editing them if needed.
4. Click **Create missing in Stripe**. This saves a draft automatically when needed, then provisions the Stripe catalog.
5. Review the returned `price_...` mappings.
6. Publish the draft. The mappings are saved automatically, so no second draft save is required.
...
<CopyBlock>{`stripe login`}</CopyBlock>
...
<CopyBlock>{`export Stripe__WebhookSecret=whsec_...`}</CopyBlock>
